...
function fullTextSearch (hook, vm, userconfigs) {

	hook.init(() => {
		if (userconfigs) {
			const { NO_DATA_TEXT, RESULT_COLOR } = userconfigs
			if (NO_DATA_TEXT) configs.NO_DATA_TEXT = NO_DATA_TEXT
			if (RESULT_COLOR) configs.RESULT_COLOR = RESULT_COLOR
		}
	})
